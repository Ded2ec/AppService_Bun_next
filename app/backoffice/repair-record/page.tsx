...
export default function Page() {
  ...
  //รับเครื่อง
  const [showModalRecive, setShowModalRecive] = useState(false);
  const [reciveCustomerName, setReciveCustomerName] = useState("");
  ...
  const handleDeviceChange = (deviceId: string) => {
    const device = (devices as any).find(
      (device: any) => device.id === parseInt(deviceId)
    );
    console.log(device);
    if (device) {
    ...
    try {
      if (id == 0) {
        const response = await axios.get(
          `${config.apiUrl}/api/repairRecord/checkDeviceSerial/${deviceSerial}`
        );
        ...
        await axios.put(
          `${config.apiUrl}/api/repairRecord/update/${id}`,
          payload
        );
        setId(0);
      }
  ...
  const openModalRecive = (repairRecordId: any) => {
    setReciveId(repairRecordId.id);
    setReciveCustomerName(repairRecordId.customerName);
    setReciveAmount(0);
    setShowModalRecive(true);
  };
  const closeModalRecive = () => {
    setShowModalRecive(false);
    setReciveId(0);
  };
  ...
  const handleRecive = async () => {
    const payload = {
            ...
            await axios.put(`${config.apiUrl}/api/repairRecord/recive`,payload);
            fetchRepairRecords();
            closeModalRecive();
            Swal.fire({
                title: "Recive",
                text: "Recive successfully",
                icon: "success",
                confirmButtonText: "OK",})
        }
  ...
  return (
    <>
            ...
            <tbody>
              {repairRecords.map((repairRecord: any, index: number) => (
                <tr key={index}>
                  ...
                  <td className="text-center" style={{ width: "350px" }}>
                    <button
                      className="btn-edit"
                      style={{ borderRadius: "8px" }}
                      onClick={() => openModalRecive(repairRecord)}
                    >
      ...
      <Modal
        title="รับเครื่อง"
        isOpen={showModalRecive}
        onClose={closeModalRecive}
        size="xl"
      >
        <div className="flex gap-4 mt-4">
          <div className="w-1/2 p-2">
            <div>ชื่อลูกค้า</div>
            <input type="text" className="from-control w-full disabled" readOnly 
            value={reciveCustomerName}/>
          </div>
            <div className="w-1/2 p-2">
                <div>จำนวนเงิน</div>
                <input type="text" className="from-control w-full text-right"
                value={reciveAmount}
                onChange={(e) => setReciveAmount(Number(e.target.value))}/>
        </div>
        </div>
        <button className="btn btn-primary mt-4" onClick={handleRecive}>
          <i className="fa-solid fa-check mr-3"></i>
          บันทึก
        </button>
      </Modal>
      <Modal
        title="เพิ่มรายการซ่อม"
        isOpen={showModal}
        onClose={closeModal}
        size="xl"
      >
